...
add_task_fields = ns.model(
    "AddTask",
    {
        "name": fields.String(required=True, example="task name", description="任务名"),
        "target": fields.String(
            required=True, example="www.freebuf.com", description="目标"
        ),
        ...
        "nuclei_scan": fields.Boolean(
            description="nuclei 扫描", example=False, default=False
        ),
        "findvhost": fields.Boolean(example=False, default=False),
        "web_info_hunter": fields.Boolean(
            example=False, default=False, description="WEB JS 中的信息收集"
        ),
    },
)
...
class ARLTask(ARLResource):
    ...
    def post(self):
        ...
        ret = {"code": 200, "message": "success", "items": task_data_list}
        return ret

        # return utils.build_ret(ErrorMsg.Success, {"items": task_data_list})


batch_stop_fields = ns.model(
    "BatchStop",
    {
...
delete_task_fields = ns.model(
    "DeleteTask",
    {
        "del_task_data": fields.Boolean(
            required=False, default=False, description="是否删除任务数据"
        ),
...
class SyncTask(ARLResource):
    ...
    def post(self):
        ...
        celerytask.arl_task.delay(options=options)  # type: ignore[attr-defined]
...
"""任务通过策略下发字段"""
task_by_policy_fields = ns.model(
    "TaskByPolicy",
    {
        "name": fields.String(description="任务名称", default=True, required=True),
        "task_tag": fields.String(
            description="任务类型标签", enum=["task", "risk_cruising"], required=True
        ),
        "target": fields.String(description="任务目标", example="", required=False),
        "policy_id": fields.String(
            description="策略 ID", example="603c65316591e73dd717d176", required=True
        ),
        "result_set_id": fields.String(
            description="结果集 ID", example="603c65316591e73dd717d176", required=False
        ),
    },
)
    ...
    def post(self):
        ...
        try:
            if task_tag == TaskTag.TASK:
                ...
                if related_scope_id:
                    ...
                    if not scope_data:
                        return utils.build_ret(
                            ErrorMsg.NotFoundScopeID, {"scope_id": related_scope_id}
                        )

                    check_target_in_scope(
                        target=target, scope_list=scope_data["scope_array"]
                    )
                ...
                if not task_data_list:
                    return utils.build_ret(
                        ErrorMsg.TaskTargetIsEmpty, {"target": target}
                    )
            ...
            if task_tag == TaskTag.RISK_CRUISING:
                if result_set_id:
                    ...
                    if target_len == 0:
                        return utils.build_ret(
                            ErrorMsg.ResultSetIsEmpty, {"result_set_id": result_set_id}
                        )
                    ...
                    if not task_data_list:
                        return utils.build_ret(
                            ErrorMsg.Error, {"result_set_id": result_set_id}
                        )
